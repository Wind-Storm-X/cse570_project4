...
def define_classifier(input_nc, ncf, ninput_edges, nclasses, opt, gpu_ids, arch, init_type, init_gain):
    ...
    if arch == 'mconvnet':
        net = MeshConvNet(norm_layer, input_nc, ncf, nclasses, ninput_edges, opt.fc_n,
                          opt.resblocks)
    elif arch == 'meshunet':
        down_convs = [input_nc] + ncf
        up_convs = ncf[::-1] + [nclasses]
        net = MeshEncoderDecoder(down_convs, up_convs, blocks=opt.resblocks,
                                 transfer_data=True)
    else:
...
class MeshConvNet(nn.Module):
    ...
    """
    def __init__(self, norm_layer, nf0, conv_res, nclasses, input_res, fc_n,
                 nresblocks=3):
...
class MeshEncoderDecoder(nn.Module):
    ...
    """
    def __init__(self, down_convs, up_convs, blocks=0, transfer_data=True):
...
class MeshEncoder(nn.Module):
    def __init__(self, convs, fcs=None, blocks=0):
